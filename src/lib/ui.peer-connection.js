...
console.log("Using WebSocket server: " + SIGNALING_SERVER);
...
rtcMultiConnection.openSignalingChannel = function(config) {
    ...
    websocket.onopen = function() {
        console.info("WebSocket connection opened: " + SIGNALING_SERVER);
...
rtcMultiConnection.onopen = function(e) {
    ...
    addNewMessage({
        ...
        message:
            "Data connection is opened between you and " +
            e.extra.username +
            ".",
...
rtcMultiConnection.onmessage = function(e) {
    ...
    addNewMessage({
        ...
        message:
            ...
            e.extra.username +
            ":<br /><br />" +
            (rtcMultiConnection.autoTranslateText
                ? linkify(e.data) + " ( " + linkify(e.original) + " )"
                : linkify(e.data)),
        userinfo: getUserinfo(
            rtcMultiConnection.blobURLs[e.userid],
...
rtcMultiConnection.onRequest = function(request) {
    ...
    addNewMessage({
        ...
        message:
            "A participant found. Accepting request of " +
            request.extra.username +
            " ( " +
            request.userid +
            " )...",
        userinfo: '<img src="images/action-needed.png">',
...
rtcMultiConnection.onCustomMessage = function(message) {
    ...
    if (message.renegotiate) {
        ...
        if (customStream) {
            rtcMultiConnection.peers[message.userid].renegotiate(
                customStream,
                message.session
            );
        }
    ...
    if (e.stream.getVideoTracks().length) {
        ...
        addNewMessage({
            ...
            userinfo:
                ...
                '" autoplay muted=true volume=0></video>',
            color: e.extra.color
        });
    } else {
        addNewMessage({
            ...
            userinfo:
                '<audio src="' +
                URL.createObjectURL(e.stream) +
                '" controls muted=true volume=0></audio>',
            color: e.extra.color
        });
    }
...
rtcMultiConnection.onlose = rtcMultiConnection.onleave = function(event) {
    addNewMessage({
        header: event.extra.username,
        message: event.extra.username + " left the room.",
        userinfo: getUserinfo(
            rtcMultiConnection.blobURLs[event.userid],
            "images/info.png"
        ),
